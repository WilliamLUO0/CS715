...
Shader "Mobile/Skybox"
{
	Properties
	{
		...
		_SunIntensity("Sun Intensity", Range(0.0,20.0)) = 10.0
		 ...
		 _MagnitudeOffset ("_MagnitudeOffset", Range (0.01, 4)) = 1.6
	   _MagnitudeDivident ("_MagnitudeDivident", Range (0.01, 20)) = 1.3
	}
...
#include "UnityCG.cginc"
	...
	uniform float _MagnitudeOffset;
	uniform float _MagnitudeDivident;
	...
	half4 frag(v2f IN) : COLOR
	{
		...
		float magnitudeNormal =  _MagnitudeOffset + abs((derivedNormal.x+derivedNormal.y+derivedNormal.z))/_MagnitudeDivident;
		...
		half3 c_sky = (_SkyColor1 * p1 + _SkyColor2 * p2)*magnitudeNormal ;
